<img width="512" height="251" alt="logo_3_ npvp" src="https://github.com/user-attachments/assets/05eb7a1d-20ef-480e-9c9e-85ce54e11609" />

[Site](https://bms-prog.github.io/#nppvoiceplugin)
[FAQ](https://sites.google.com/view/nppvoiceplugin/frequently-asked-questions)


# NppVoicePlugin


## EN

📝 Boost your productivity! Dictate text, read selected text aloud, and send spoken or selected text directly to your local Ollama AI.
Completely local solution.

**Features:**
* Voice typing with speech recognition
* Text-to-speech
* Send spoken or selected text to your local Ollama AI
* Supports English and Russian interface languages ​​and 19 voice model languages.
* Seamless integration with Notepad++
* The ability to use text files as context for AI.
* For detailed instructions on installing components, configuring, and using them, please see the Help section in the plugin menu.


🛠️ **Installation:**

Install from the Notepad++ plugin manager or unzip NppVoicePlugin.zip into the Notepad++ plugins folder if installing manually.

Path: **Plugins → Open Plugins Folder** in the Notepad++ menu.

Example:
`C:\Program Files\Notepad++\plugins\NppVoicePlugin\`


⚠️ After installation, read the Help in the NppVoicePlugin plugin menu and install the remaining components.


## RU

📝 Повысьте свою производительность с помощью голосового ввода и интеграции с ИИ! Диктуйте текст, читайте выделенный текст вслух и отправляйте произнесенный или выделенный текст непосредственно в локальный ИИ Ollama.
Полностью локальное решение.


**Возможности программы:**
* Голосовой ввод с распознаванием речи
* Преобразование текста в речь
* Отправка произнесенного или выделенного текста в локальный ИИ Ollama
* Поддерживает английский и русский языки интерфейса и 19 языков голосовых моделей.
* Бесшовная интеграция с Notepad++
* Использование текстовых файлов как контекста для ИИ
* Подробные инструкции по установке компонентов, настройке и использованию см. в разделе «Справка» в меню плагина.


🛠️ **Установка:**
Установите из менеджера плагинов Notepad++  или распакуйте NppVoicePlugin.zip в папку плагинов Notepad++ при ручной установке.

Путь: Плагины > Открыть папку плагинов в меню Notepad++.

Например: 
`C:\Program Files\Notepad++\plugins\NppVoicePlugin\`


⚠️ После установки прочитайте Помощь в меню плагина NppVoicePlugin и установите остальные компоненты.


# Versions

v.1.5
- Added msvcp140.dll, vcruntime140.dll, vcruntime140_1.dll (release VC++ 2015-2022
Redistributable) to the plugin package — fixes "The specified module could not
be found" error when running without administrator rights and without the
VC++ Redistributable installed on the system.

v.1.4
- Added the ability to use text files as context for AI.
- Updated help files.

v.1.3
- Added the ability to send a voice message to Ollama along with selected text as context.
For example:
Select 2+2 in the text and say "Count"
Select previous questions and answers to Ollama and ask a follow-up question using voice.
- Changed the status text after sending a text to Ollama
- Fixed the way Ollama displays a response after a selected block of text.

v.1.2
- The plugin is made universal for all available voice models.

v.1.1
- Public release for Notepad++ v8.8.8 and above
